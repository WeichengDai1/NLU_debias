...
class MyAllDataset():
    ...
    def Read_Data(self):
        ...
        def Init_Public(train_examples, dev_examples, test_examples):
            ...
            keywords_type_count= {k:0 for k in ['PER', 'LOC', 'ORG', 'MISC']}
            total_word_counts = 0
            for i, example in enumerate(examples):
                ...
                # '''
                # # sentence = "Dena gave me a flue shot in New York"
                ...
                # print(f" ********* original method: keywords = {keywords}")
                '''
                first NER
                '''
                # ner_results = nlp(sentence)
                # print(f" ********* First ner_results = {ner_results}")
                ...
                keywords = list(sentence.get_spans('ner'))
                # print('********* Second NER keywords: ', keywords) 
                
                total_word_counts += len(example.text)
                keywords_map = {}
                for item in keywords:
                    item = str(item) # Span[0:1]: "Dena" → PER (0.9942)
                    found = item.split(":")[-1].split('→') # 0:  [' "Dena" ', ' PER (0.9942)']
                    found[0] = found[0].split('"')[1] ## "Dena" -> Dena      
                    identity_type = found[1].strip().split('(')[0].strip()
                    found[1] = float(found[1].split('(')[1].strip(')')) # 0.9942
                    for sub_word in found[0].split(' '):
                        keywords_map[sub_word] = found[1] 
                        keywords_type_count[identity_type]+=1 ## New and York count twice of LOC
                # print(keywords_map) # {'Dena': 0.9942, 'New York': 0.9952}
                # print(keywords_type_count) # {'PER': 1, 'LOC': 2, 'ORG': 0, 'MISC': 0}
